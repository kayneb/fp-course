...
unexpectedStringParser ::
  Chars
  -> Parser a
unexpectedStringParser s =
  P (\_ -> UnexpectedString s)
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P primary) (P secondary) = P(\i ->
    let primaryResult = primary i
    in if isErrorResult primaryResult then secondary i else primaryResult
  )
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser ps =
  foldRight (\pa plt -> bindParser (\a -> mapParser (a :.) plt) pa) (valueParser Nil) ps
...
firstNameParser ::
  Parser Chars
firstNameParser =
  bindParser (\u -> mapParser (u :.) (list lower)) (bindParser valueParser upper)
...
surnameParser ::
  Parser Chars
surnameParser =
  bindParser (\ul -> mapParser (ul ++) (list lower)) (bindParser (\u -> mapParser (u :.) (thisMany 5 lower)) (bindParser valueParser upper))
...
smokerParser ::
  Parser Char
smokerParser =
  bindParser valueParser (is 'y') ||| bindParser valueParser (is 'n')
...
phoneBodyParser ::
  Parser Chars
phoneBodyParser =
  list (bindParser valueParser (is '-') ||| bindParser valueParser (is '.') ||| bindParser valueParser digit)
...
phoneParser ::
  Parser Chars
phoneParser =
  bindParser (\p -> mapParser (\_ -> p) (bindParser valueParser (is '#'))) (bindParser (\d -> mapParser (d :.) phoneBodyParser) (bindParser valueParser digit))
