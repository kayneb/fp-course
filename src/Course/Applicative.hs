...
instance Applicative ExactlyOne where
  ...
  (<*>) :: 
    ...
    -> ExactlyOne a
    -> ExactlyOne b
  (<*>) (ExactlyOne f) (ExactlyOne fa) = ExactlyOne(f fa)
...
lift5 ::
  ...
  -> f e
  -> f f1
lift5 f fa fb fc fd fe = lift4 f fa fb fc fd <*> fe
...
filtering ::
  ...
  -> f (List a)
filtering f =
  foldRight (\v facc -> lift2 (\b acc -> if b then v :. acc else acc) (f v) facc) (pure Nil)
