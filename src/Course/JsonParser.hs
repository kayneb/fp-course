...
jsonString ::
  Parser Chars
jsonString =
  let nonHexSpecialChar = ((\c -> ((valueParser . fromSpecialCharacter) <$> (toSpecialCharacter c)) ?? unexpectedCharParser c) =<< character)
  in between (is '"') (charTok '"') (list ((\c -> if c == '\\' then hexu ||| nonHexSpecialChar else valueParser c) =<< (satisfy (/= '"'))))
...
jsonNumber ::
  Parser Rational
jsonNumber =
  let processRealFracAndRemainder = \rf ->
                                      case snd rf of 
                                        Nil -> valueParser (fst rf)
                                        _ -> unexpectedStringParser (snd rf)
  in (\s -> (processRealFracAndRemainder <$> (readFloats s)) ?? unexpectedStringParser s) =<< (list1 (oneof "-1234567890"))
...
jsonObject ::
  Parser Assoc
jsonObject =
  betweenSepbyComma '{' '}' (jsonString >>= (\key -> charTok ':' *> jsonValue >>= (\val -> valueParser (key, val))))
